...
void transceiver_interface(
    ...
    real_T blood_speed, real_T mod_order_int, const real_T bits[8000],
    real_T bit_rate, real_T A, real_T Const_Tx_real[8000],
    real_T Const_Tx_imag[8000], real_T Const_Rx_real[8000],
    real_T Const_Rx_imag[8000], real_T nanosensor_pos_x[8000])
{
  (void)sp;
  memset(&Const_Tx_real[0], 0, 8000U * sizeof(real_T));
  memset(&Const_Tx_imag[0], 0, 8000U * sizeof(real_T));
  transceiver(Freq_THz, dist_init_x, Skin_thickness, Tissue_thickness,
              Vessel_thickness, blood_speed, mod_order_int, &bits[0], 8000.0,
