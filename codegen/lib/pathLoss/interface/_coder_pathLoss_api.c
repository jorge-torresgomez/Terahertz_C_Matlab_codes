...
void pathLoss_api(const mxArray *const prhs[5], const mxArray **plhs)
{
  ...
  Freq_THz = pathLoss(Freq_THz, Comm_dist, Skin_thickness, Tissue_thickness,
                      Vessel_thickness);
...
void pathLoss_atexit(void)
{
  ...
  pathLoss_xil_terminate();
  pathLoss_xil_shutdown();
...
/*
 * File trailer for _coder_pathLoss_api.c
 *
 * [EOF]
 */
